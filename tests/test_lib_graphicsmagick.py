"""Tests for the GraphicsMagick backend (imgfind.lib.graphicsmagick)."""
import shutil
import pytest
...
pytestmark = pytest.mark.skipif(
    shutil.which('gm') is None,
    reason='GraphicsMagick (gm) not available',
)


@pytest.fixture(autouse=True)
def clear_cache():
    from imgfind.lib.graphicsmagick import img_format
...
def test_convert_jpeg_to_webp(tmp_path):
    from imgfind.lib.graphicsmagick import convert
...
def test_alpha_used_with_alpha(tmp_path):
    from imgfind.lib.graphicsmagick import alpha_used
    p = _make_png(tmp_path / 'test.png', alpha=True)
    # Image has semi-transparent pixels (alpha=180), so alpha is used
...
def test_alpha_used_without_alpha(tmp_path):
    ...
    # Image has no alpha channel, so alpha is not used
    result = alpha_used(str(p))
